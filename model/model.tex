...
If there is only one occupation and there is no correlation between cities, $\rho_k = 0$, for that occupation, the city choice share will reduce down to the following:
...
\begin{align*}
    \frac{\partial ln \pi_{ck}}{\partial T_{c'k}^{\star}} & = - \frac{1}{T_{ck}^{\star}} \frac{(T_{c'k}^{\star} Z_{c'}^{- \theta})^{\frac{1}{1 - \rho_k}}}{\sum_{c}^{N} (T_{ck}^{\star} Z_c^{-\theta})^{\frac{1}{1 - \rho_k}}} < 0 \\
\end{align*}
