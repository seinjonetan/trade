...
With the case of a cross-nested CES function, we can approximate a correlation funciton. Assuming that productivity is distributed max-stable multivariate Fréchet, with scale parameter $T_{ck}^{\star} H_k$, shape parameter $\theta$, and correlation parameter $\rho_k$, we can assume the following correlation function:

\begin{align}
    G (Z_1^{-\theta}, \dots, Z_N^{-\theta})= \sum_{k}^{} [\sum_{c}^{N} (T_{ck}^{\star} H_k Z_c^{-\theta})^{\frac{1}{1 - \rho_k}}]^{1 - \rho_k}
\end{align}
...
\begin{align*}
    G_c (Z_1^{-\theta}, \dots, Z_N^{-\theta})               & = \frac{\partial G(Z_1^{-\theta}, \dots, Z_N^{-\theta})}{\partial Z_c^{-\theta}}                                    \\
                                                            & = \sum_{k}^{} [(T_{ck}^{\star} H_k)^{\frac{1}{1 - \rho_k}} (Z_c^{-\theta})^{\frac{1}{1 - \rho_k} - 1}]^{1 - \rho_k} \\
                                                            & = Z_c^{-\theta} \sum_{k}^{} [(T_{ck}^{\star} H_k Z_c^{-\theta} )^{\frac{1}{1 - \rho_k}}]^{1 - \rho_k}               \\
                                                            & = Z_c^{-\theta} \sum_{k}^{} T_{ck}^{\star} H_k Z_c^{-\theta}                                                        \\
    Z_c^{-\theta} G_c (Z_1^{-\theta}, \dots, Z_N^{-\theta}) & = \sum_{k}^{} T_{ck}^{\star} H_k Z_c^{-\theta}
\end{align*}

\begin{align*}
    \frac{Z_c^{-\theta} G(Z_1^{-\theta}, \dots, Z_N^{-\theta})}{G(Z_1^{-\theta}, \dots, Z_N^{-\theta})} & = \frac{\sum_{k}^{} T_{ck}^{\star} H_k Z_c^{-\theta}}{\sum_{k}^{} [\sum_{c}^{N} (T_{ck}^{\star} H_k Z_c^{-\theta})^{\frac{1}{1 - \rho_k}}]^{1 - \rho_k}}      \\
                                                                                                        & = \frac{\sum_{k}^{} T_{ck}^{\star} H_k Z_c^{-\theta}}{\sum_{c}^{N} \sum_{k}^{} (T_{ck}^{\star} H_k Z_c^{-\theta})^{\frac{1}{1 - \rho_k} \times (1 - \rho_k)}} \\
                                                                                                        & = \frac{T_c^{\star} H^{\star} Z_c^{-\theta}}{\sum_{c}^{N} T_c^{\star} H^{\star} Z_c^{-\theta}}                                                                \\
