...
We assume no trade costs between cities, and that the price of the good produced by sector $k$ is freely traded and priced under perfect competition. The price of the good produced by sector $k$ is $p_k$. The wage of a worker in sector $k$ in city $c$ is $w_{ck}(\nu) = p_k Z_{ck}(\nu)$. As in EK, productivity is a random variable drawn from a max stable multivariate Fréchet distribution and is dependent on both the city and occupation. Output by a specifc occupation is assumed to be produced by a random sector.
